-- Migration: 20260917120000_fix_admin_portal_snapshot_remove_legacy_ocr.sql
-- Description: Fix regression in get_admin_portal_snapshot caused by 20260916160000
--              which erroneously reintroduced legacy OCR column references
--              (ocr_text, ocr_status, ocr_confidence, ocr_metadata) that were dropped
--              in 20260911200000_remove_legacy_ocr.sql.
--              Maintains archived templates inclusion (no "where rdt.is_active = true")
--              introduced on September 16, 2026.
