-- Migration: 20260911200000_remove_legacy_ocr.sql
-- Purpose: Complete removal of legacy OCR references and database columns.
-- 1. Redefines get_admin_portal_snapshot() without legacy OCR fields (ocr_text, ocr_status, ocr_confidence, ocr_metadata).
-- 2. Drops the legacy OCR columns from public.document_submission_files.

-- Step 1: Update RPC to remove column references before dropping columns
...
-- Step 2: Safely drop legacy OCR columns now that no active RPCs or queries reference them
