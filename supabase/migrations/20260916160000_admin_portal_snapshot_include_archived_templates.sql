-- Migration: 20260916160000_admin_portal_snapshot_include_archived_templates.sql
-- Description: Update get_admin_portal_snapshot to return both active and archived templates
--              (removing "where rdt.is_active = true" from the templates subquery)
--              so that the Admin Forms & Templates page can display, filter, and restore
--              archived templates. Public/user-facing portals continue filtering active-only.
...
begin
  ...
  return jsonb_build_object(
    ...
    coalesce(
      (
        -- Dropped "where rdt.is_active = true" so the admin portal snapshot
