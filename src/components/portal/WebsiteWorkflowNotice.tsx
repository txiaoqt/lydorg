...
export const WebsiteWorkflowNotice: React.FC<WebsiteWorkflowNoticeProps> = ({
  ...
  <Card className="border-amber-300/70 bg-amber-50/70 dark:bg-amber-950/20 dark:border-amber-700/50 shadow-sm rounded-2xl">
    <CardContent className="p-5 sm:p-6">
      <div className="flex items-start gap-3">
        <div className="rounded-xl bg-amber-100 dark:bg-amber-900/40 p-2 text-amber-700 dark:text-amber-300 shrink-0">
          <ClipboardList className="h-5 w-5" aria-hidden="true" />
        </div>
        <div className="min-w-0 flex-1">
          <h2 className="text-base sm:text-lg font-bold text-amber-950 dark:text-amber-100">{title}</h2>
          <p className="mt-1 text-xs sm:text-sm text-amber-900/90 dark:text-amber-200/80 leading-relaxed">{description}</p>
          ...
          <Button
            type="button"
            variant="ghost"
            className="mt-4 h-9 px-0 text-xs sm:text-sm font-bold text-amber-800 dark:text-amber-300 hover:bg-transparent hover:text-amber-950 dark:hover:text-amber-100 cursor-pointer"
            onClick={onAction}
          >
            {actionLabel}
