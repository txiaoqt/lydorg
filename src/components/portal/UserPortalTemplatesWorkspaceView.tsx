...
export const extractTemplateCategoryLabels = (tpl: any): string[] => {
  const rawList = Array.isArray(tpl?.categories) && tpl.categories.length > 0
    ? tpl.categories
    : Array.isArray(tpl?.templateCategories) && tpl.templateCategories.length > 0
    ? tpl.templateCategories
    : Array.isArray(tpl?.rawCategories) && tpl.rawCategories.length > 0
    ? tpl.rawCategories
    : tpl?.category
    ? [tpl.category]
    : [];
  ...
  rawList.forEach((cat: any) => {
    if (typeof cat === "string" && cat.trim()) {
      labels.add(formatTemplateCategoryDropdownLabel(cat.trim()));
    }
  });

  if (labels.size === 0 && (tpl?.title || tpl?.name)) {
    const derived = deriveTemplateCategory(tpl.title || tpl.name);
    if (derived) {
      labels.add(formatTemplateCategoryDropdownLabel(derived));
    }
  }
...
export const UserPortalTemplatesWorkspaceView: React.FC<UserPortalTemplatesWorkspaceViewProps> = ({
  ...
  const availableCategories = useMemo(() => {
    ...
    publicTemplates.forEach((t) => {
      const labels = extractTemplateCategoryLabels(t);
      labels.forEach((l) => cats.add(l));
    });
    return orderTemplateCategories(Array.from(cats));
  }, [publicTemplates]);
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return publicTemplates.filter((tpl) => {
      const labels = extractTemplateCategoryLabels(tpl);
      const matchesSearch =
        !query ||
      ...
      const matchesCategory =
        ...
        labels.some(
          (l) =>
            l.toLowerCase() === categoryFilter.toLowerCase() ||
            l.toLowerCase().replace(/[\s-]+/g, "_") === categoryFilter.toLowerCase().replace(/[\s-]+/g, "_")
        );
  ...
  const templateMatchesCategory = (tpl: any, targetCategory: string): boolean => {
    const labels = extractTemplateCategoryLabels(tpl);
    const targetKey = targetCategory.trim().toLowerCase().replace(/[\s-]+/g, "_");
    return labels.some((label) => {
      const labelKey = label.trim().toLowerCase().replace(/[\s-]+/g, "_");
      return labelKey === targetKey || label.toLowerCase() === targetCategory.toLowerCase();
    });
  };

  // Dynamic Category-Driven Groups matching Admin grouping model
  const categoryGroups = useMemo(() => {
    const targetCategories =
      categoryFilter !== "all"
        ? [availableCategories.find((c) => c.toLowerCase() === categoryFilter.toLowerCase()) || categoryFilter]
        : availableCategories;

    return targetCategories
      .map((category) => ({
        category,
        items: filteredTemplates.filter((tpl) => templateMatchesCategory(tpl, category)),
      }))
      .filter((group) => group.items.length > 0);
  }, [availableCategories, filteredTemplates, categoryFilter]);
  ...
  return (
    <div className="bg-background text-foreground transition-colors duration-200 font-sans max-w-[1440px] mx-auto pt-0 pb-2 sm:py-2">
      ...
      <div className="mobile-layout block lg:hidden space-y-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-56 p-1.5 rounded-xl bg-card border-border/80 shadow-lg">
                ...
                {availableCategories.map((cat) => {
                  const count = publicTemplates.filter((t) => {
                    const labels = extractTemplateCategoryLabels(t);
                    return labels.some(
                      (l) =>
                        l.toLowerCase() === cat.toLowerCase() ||
                        l.toLowerCase().replace(/[\s-]+/g, "_") === cat.toLowerCase().replace(/[\s-]+/g, "_")
                    );
                  }).length;
                  return (
                    <DropdownMenuItem
        ...
        {/* Dynamic Category Sections (Mobile) */}
        {categoryGroups.map((group) => {
          const isCollapsed = collapsedCategories.has(group.category);
          const requiredCount = group.items.filter((t) => t.isRequired).length;
          const sectionZipKey = `mobile-section-${group.category}`;
          const sectionZipFileName = `${group.category.replace(/[\s/]+/g, "-")}-Templates.zip`;
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  >
                    ...
                    <span className="text-[10px] text-muted-foreground font-semibold block">
                      {requiredCount > 0
                        ? `${requiredCount} Required • ${group.items.length} Total`
                        : `${group.items.length} Supplementary Document${group.items.length > 1 ? "s" : ""}`}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0" onClick={(e) => e.stopPropagation()}>
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      disabled={downloadingZipKey === sectionZipKey}
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    >
                      {downloadingZipKey === sectionZipKey ? (
                        <>
                        ...
                        <>
                          <FolderArchive className="h-3.5 w-3.5 text-primary shrink-0" />
                          <span>ZIP Section ({group.items.length})</span>
                        </>
                      )}
                    </Button>
                    ...
                    <button
                      ...
                      aria-label={`Toggle ${group.category} templates section`}
                    >
                      <ChevronDown
                        className={cn(
              ...
              {!isCollapsed && (
                <div className="space-y-3">
                  {group.items.map((tpl) => {
                    ...
                    return (
                      <Card
                        key={`${group.category}-${tpl.id}`}
                        className="rounded-2xl border border-border/60 bg-card p-4 space-y-3 shadow-xs hover:border-primary/40 transition-all flex flex-col"
                      >
                        {/* Top Row: Icon + Title */}
                          ...
                          <div className="min-w-0 flex-1 space-y-1">
                            ...
                            <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                              ...
                              {extractTemplateCategoryLabels(tpl).map((cat) => (
                                <span
                                  key={cat}
                                  className="text-[10px] font-medium bg-muted/60 border border-border/50 px-2 py-0.5 rounded-md text-muted-foreground truncate max-w-[140px]"
                                >
                                  {cat}
                                </span>
                              ))}
      ...
      <div className="desktop-layout hidden lg:block space-y-6">
              ...
              <DropdownMenuContent align="end" className="w-52 p-2 rounded-xl bg-card border-border/80">
                ...
                {availableCategories.map((cat) => {
                  ...
                  return (
                    ...
                    >
                      <span>{cat}</span>
                      {count > 0 && <span className="text-[10px] text-muted-foreground ml-2">({count})</span>}
                    </DropdownMenuItem>
                  );
                })}
              </DropdownMenuContent>
        ...
        {/* Dynamic Category Sections (Desktop) */}
        {categoryGroups.map((group) => {
          const isCollapsed = collapsedCategories.has(group.category);
          const requiredCount = group.items.filter((t) => t.isRequired).length;
          const sectionZipKey = `desktop-section-${group.category}`;
          const sectionZipFileName = `${group.category.replace(/[\s/]+/g, "-")}-Templates.zip`;

          return (
            <Card
              key={group.category}
              className="rounded-2xl border border-border/60 bg-card overflow-hidden shadow-xs space-y-0"
            >
              <div
                onClick={() => toggleCategoryCollapse(group.category)}
                ...
                <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                  ...
                  >
                    {downloadingZipKey === sectionZipKey ? (
                      ...
                      <>
                        <FolderArchive className="h-3 w-3 text-primary" /> ZIP Section ({group.items.length})
                      </>
                    )}
                  </Button>
                  <span className="text-[11px] text-muted-foreground font-semibold">
                    {requiredCount > 0 ? `${requiredCount} Required` : "Supplementary"}
                  </span>
                </div>
              </div>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div className="space-y-0.5 min-w-0">
                                  <div className="flex items-center gap-2">
                                    ...
                                    {tpl.isRequired ? (
                                      <span className="text-[9px] font-semibold bg-primary/10 text-primary px-1.5 py-0.5 rounded-sm shrink-0">
                                        Required
                                      </span>
                                    ) : (
                                      <span className="text-[9px] font-medium bg-muted text-muted-foreground px-1.5 py-0.5 rounded-sm shrink-0">
                                        Optional
                                      </span>
                                    )}
                                  </div>
