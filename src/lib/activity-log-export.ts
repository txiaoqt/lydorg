...
export const activityLogExportConfig: ReportExportConfig<ExportActivityRow> = {
  ...
  footerText: "Pasig City Youth Development Office - Activity Logs",
  xlsxSheetName: "Activity Logs",
  pdfUseUnicodeFont: true,
  pdfFontSize: 7,
  pdfCellPadding: 3.5,
  columns: [
    { label: "Log ID", value: (row) => row.logId, pdfWidth: 44, pdfAlign: "center", xlsxWidth: 24 },
    { label: "Date", value: (row) => row.date, xlsxValue: (row) => row.timestamp, xlsxType: "date", pdfWidth: 46, pdfAlign: "center", xlsxWidth: 16 },
    { label: "Time", value: (row) => row.time, pdfWidth: 38, pdfAlign: "center", xlsxWidth: 14 },
    { label: "Action", value: (row) => row.action, pdfWidth: 68, xlsxWidth: 30, xlsxWrap: true },
    { label: "Category", value: (row) => row.category, pdfWidth: 45, xlsxWidth: 20 },
    { label: "Description", value: (row) => row.description, csvPreserveLineBreaks: true, pdfWidth: 130, xlsxWidth: 55, xlsxWrap: true },
    { label: "Affected Record", value: (row) => row.affectedRecord, pdfWidth: 52, xlsxWidth: 35, xlsxWrap: true },
    { label: "Actor", value: (row) => row.actor, pdfWidth: 56, xlsxWidth: 24 },
    { label: "Organization", value: (row) => row.organization, pdfWidth: 56, xlsxWidth: 28, xlsxWrap: true },
  ],
};
