...
export const ReplyEmailDialog = ({
  ...
  const handleMarkReviewed = async (action: "email" | "status") => {
  ...
  const handleOpenEmail = async () => {
    ...
    await handleMarkReviewed("email");
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="flex flex-1 flex-col gap-4">
          {isAlreadyReviewed ? (
            <div className="flex items-start gap-2.5 rounded-md border border-slate-300 bg-bg-panel-subtle p-6">
              <CheckCircle className="mt-0.5 h-4 w-4 shrink-0 text-public-bg-brand" strokeWidth={1.6} />
              <p className="font-segoe text-[13px] font-normal leading-[120%] text-text-default">
        ...
        <div className="flex flex-col gap-2.5">
          {!isAlreadyReviewed ? (
            <>
              ...
              >
                ...
                Open Email &amp; Mark as Reviewed
              </button>
              <button
                type="button"
                onClick={() => void handleMarkReviewed("status")}
                disabled={savingAction !== null}
                className={cn(
          ...
          <button
            ...
            disabled={savingAction !== null}
            className={cn(
              "h-11 w-full rounded-md border border-slate-300 bg-admin-surface px-4 py-3 font-segoe text-public-fs-body-sm font-normal leading-[140%] text-text-default transition-colors disabled:opacity-50",
              "hover:bg-slate-50",
            )}
          >
